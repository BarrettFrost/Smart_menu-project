...
We also thought through how the system could be implemented and could connected be with each other. The Web application has been designed as a service provider who can send the menu to the M5. Desktop app, as the restaurant side, would send the menu and update to the web service provider.  We designed some functions for example adding the new meal and updating the menu, showing the main page of all the dishes, and registering a new restaurant. The desktop user interface design *below* could helped us test some ideas to build the best system.
...
When experimenting with MQTT and M5 a problem was discovered, the M5 could only accept a certain number of characters via JSON from MQTT. This was issue because our JSON String we quite long due to all menu items and each indivual items data. With some research it was discovered that the PubSubClient library has a max MQTT JSON Packet size of 256. This was subsequently increased to 1000. By the end both the m5 and web app were capable of sending dummy text to the mqtt broker by the end of the sprint.
...
Two goals were achieved in our fourth sprint. First, we began UI work and debugging on the m5 and Web App. Second, We began to work on the Desktop Application, meant to represent the restaurant.
...
We conducted our first tests between the M5 and Web Application. During the test, we fixed minor bugs, such as oversights in capitalization for variables between the different environments (e.g. "resName" was written as "ResName" on the Web Application's JSONpacket, which would not parse properly). That aside, testing went smoothly.
...
- [the restaurant user] wants to easily edit their data, so using the service will not be a hassle to use on a day to day basis.

We knew that each restaurant had to have a unique key. Relying on restaurant name *resName* wasn't sufficient as restaurants could have similar names. It is also good practice to not have the key be user editable as it can lead to database corruption. So we added a random read-only alphanumeric key generator on the Web Application that would generate and return *resID* whenever a restaurant registers with the service.

#### *Sprint 5: Polishing, Full project testing (Late March - April)*

This (final) sprint encapsulates our several weeks of final testing, debugging and polishing.

Notable changes made during this stage was adding the *conID* variable. We realized in our testing that we had overlooked the need to identify unique conversations between devices. This was to allow multiple simultaneous conversations to happen without disrupting each other (without it, one M5 requesting the restaurant list would cause all M5s on the network to receive the response and switch to the restaurant list page).

*conID* was a random 4-digit integer generated on the M5 whenever it sent a query. Similar to *resID*, we made *conID* a read-only variable to prevent tampering. To pair a query to the right response, the Web Application simply copies the ID from the inbound query to the outbound response. the M5 also forgets the ID at the end of the conversation.
...
=======
>>>>>>> d9ca8463b28a91fdc518dd41b095538afe79b018
